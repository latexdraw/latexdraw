...
abstract class LAbstractGrid extends LPositionShape implements IStandardGrid {
	...
	public boolean setBottom(final double y) {
		...
		if(y<getTopLeftPoint().getY()) {
			double newEnd 	= gridStarty + (pos.getY() - y)/step;
			pos.setY(pos.getY()-step*(gridEndy-gridStarty));
			gridStarty 		= gridEndy;
			gridEndy   		= newEnd;
		}else {
			gridStarty = gridStarty - (y - pos.getY())/step;
			pos.setY(y);
		}
	...
	public boolean setLeft(final double x) {
		...
		if(x>getTopRightPoint().getX()) {
			double newEnd 	= gridStartx + (x - pos.getX())/step;
			pos.setX(pos.getX()+step*(gridEndx-gridStartx));
			gridStartx 		= gridEndx;
			gridEndx   		= newEnd;
		}else {
			gridStartx = gridStartx - (pos.getX() - x)/step;
			pos.setX(x);
		}
	...
	public boolean setRight(final double x) {
		...
		if(x<pos.getX()) {
			gridEndx   = gridStartx;
			gridStartx = gridEndx - (pos.getX() - x)/step;
			pos.setX(pos.getX() - (gridEndx-gridStartx)*step);
		}else
			gridEndx = gridStartx + (x - pos.getX())/step;
	...
	public boolean setTop(final double y) {
		...
		if(y>pos.getY()) {
			gridEndy   = gridStarty;
			gridStarty = gridEndy - (y - pos.getY())/step;
			pos.setY(pos.getY() + (gridEndy-gridStarty)*step);
		}else
			gridEndy = gridStarty + (pos.getY() - y)/step;
