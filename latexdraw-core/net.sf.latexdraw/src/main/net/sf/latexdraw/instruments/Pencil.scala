...
class Pencil(canvas : ICanvas, val textSetter:TextSetter, val layers:MLayeredPane) extends CanvasInstrument(canvas) {
	...
	protected lazy val _groupParams = {
		...
		val fac = DrawingTK.getFactory
		group.addShape(fac.createRectangle(false))
		group.addShape(fac.createDot(fac.createPoint, false))
		group.addShape(fac.createGrid(false, fac.createPoint))
		group.addShape(fac.createAxes(false, fac.createPoint))
		group.addShape(fac.createText(false))
		group.addShape(fac.createCircleArc(false))
		group.addShape(fac.createPolyline(false))
		group.addShape(fac.createBezierCurve(false))
		group.addShape(fac.createFreeHand(false))
		group
	}
				...
				shape match {
					...
					case _ =>
						if(ec==EditionChoice.FREE_HAND && shape.isInstanceOf[IFreehand])
							shape.asInstanceOf[IFreehand].addPoint(DrawingTK.getFactory.createPoint(pt.getX, pt.getY))
						else
...
private sealed class Press2AddText(pencil:Pencil) extends PencilLink[Press](pencil, false, classOf[Press]) {
	override def initAction() {
		action.setDrawing(instrument.canvas.getDrawing)
		action.setShape(DrawingTK.getFactory.createText(true, DrawingTK.getFactory.createPoint(instrument.textSetter.relativePoint), instrument.textSetter.getTextField.getText))
	}
