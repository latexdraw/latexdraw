...
object LShapeFactory {
	...
	implicit def Point2IPoint(pt:Point) = ShapeFactory.factory.createPoint(pt.getX, pt.getY)

	/** Converts a Point2D to an IPoint. */
	implicit def Point2D2IPoint(pt:Point2D) = ShapeFactory.factory.createPoint(pt.getX, pt.getY)
